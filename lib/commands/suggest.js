...
async function cmdSuggest(ctx, args) {
  ...
  if (!awemeId) throw new Error('用法: node cli.js suggest <aweme_id> [--auto] [--min-priority N] [--mode text-only|screenshots|video] [--screenshots N] [--fast] [--force]');
  ...
  // ── 多模态配置 ──
  const mode = getFlag(args, '--mode', 'text-only'); // text-only | screenshots | video
  ...
  if (effectiveMode !== 'text-only') {
    ...
    console.error(`[suggest] 多模态模式: ${effectiveMode} (截图数: ${screenshotCount})`);
